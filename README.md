...
| 指标 | 参数 | 用途 |
|------|------|------|
| MA | 5/10/20/30/60 | 趋势方向与启动点结构 |
| MACD | 12/26/9 EMA | 动量信号 |
| ENE | 10 日均线，±11%/±9% | 通道位置与向上穿越确认 |
| DMA | DDD=MA(C,10)-MA(C,50)，AMA=DDD 的 10 日均线 | 中期动量交叉确认 |
| RSI | 6/14 | 超买/超卖 |
...
股票启动点策略统一使用前复权收盘价计算 MA、ENE、DMA、MACD 和成交量条件，避免分红除权造成的虚假穿越。买入信号按以下顺序确认：

1. **候选点**：MA20/MA30 斜率不在下跌通道，且 MA20/MA30 高于 MA5/MA10；随后 MA5 向上穿过 MA10，并通过成交量放大确认。
2. **ENE 确认**：价格位于 ENE 上下轨之间或靠近下轨，并向上穿过 ENE 中轨。
3. **DMA 确认**：DDD 向上穿过 AMA。
4. **MACD 确认**：DIF 向上穿过 DEA，且 MACD 柱线继续改善。四步需在确认窗口内按顺序发生，最后一步作为买入点。

卖出采用分层风控：跌破买入价的硬止损优先；盈利后从阶段高点回撤触发移动止盈；正常退出要求 MA 结构、DMA、MACD、长期均线结构中至少两项转弱并连续确认两日，或价格跌破 ENE 下轨。对应实现和信号明细见 `calc_startup_strategy` 及 `/api/strategy/startup/stock/{code}`。
